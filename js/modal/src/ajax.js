export class Ajax{
  constructor(options){
    ...
    xhr.open(options.method || 'get' , options.url || getApiUrl() , true)
    xhr.setRequestHeader('Content-Type', this.getContentType(options))
    xhr.onload = options.callback || null
    xhr.send(this.getQuery(options))
  }
  getQuery(options){
    return options.query ? Object.entries(options.query).map((e)=>`${e[0]}=${e[1]}`).join('&') : ''
  }
  getContentType(options){
    return options.contentText || 'application/x-www-form-urlencoded'
  }
}
